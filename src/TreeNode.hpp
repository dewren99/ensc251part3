...
class TreeNode {
 ...
 public:
  virtual ~TreeNode() {
    ...
    OUT << "Destroying TreeNode with " << childSPVector.size() << " children."
        << std::endl;
  }
  TreeNode(){};
  ...
  void traverse_children_in_order(traverse_func tf, action_func af) {
    if (childSPVector.size() != 2) {
      OUT << "Error -- can only do inorder traversal on a node with 2 children"
          << std::endl;
    }
    // ***** Complete this member function *****
